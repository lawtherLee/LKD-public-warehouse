<template>
  ...
  <el-dialog title="新增工单" round :visible.sync="dislogVisible" :before-close="hanleClose" width="40%">
    <el-form ref="addRold" :model="form" :rules="rules" size="small">
      <el-form-item
        prop="name"
        label="设备编号:"
      ...
      >
        <el-select
          ...
          style="width: 90%;"
        >
          <el-option label="补货工单" value="补货工单" />

        </el-select>
      </el-form-item>
      <el-form-item label="补货数量:" :label-width="formLabelWidth">
        <span class="blue"><i class="el-icon-document" />补货清单   </span>
      </el-form-item>
      <!--  -->
      <el-form-item
        label="运营人员:"
    ...
    <div slot="footer" class="dialog-footer">
      ...
      <el-button class="add" @click="submit">确 定</el-button>
    </div>
  </el-dialog>
</template>
...
export default {
  ...
  methods: {
    ...
    async  submit() {
      try {
        await this.$refs.addRold.validate()
      } catch (error) {
        console.log(error)
      }
    }
